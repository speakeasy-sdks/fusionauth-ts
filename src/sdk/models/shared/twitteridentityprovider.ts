...
export class TwitterIdentityProviderData extends SpeakeasyBase {}
...
export class TwitterIdentityProvider extends SpeakeasyBase {
    ...
    @SpeakeasyMetadata({ elemType: TwitterIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, TwitterIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], TwitterIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, TwitterIdentityProviderData>;
