...
export class XboxIdentityProviderData extends SpeakeasyBase {}

/**
 * Xbox gaming login provider.
 */
export class XboxIdentityProvider extends SpeakeasyBase {
    @SpeakeasyMetadata({ elemType: XboxApplicationConfiguration })
    @Expose({ name: "applicationConfiguration" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, XboxApplicationConfiguration> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], XboxApplicationConfiguration);
            }
            return obj;
        },
        { toClassOnly: true }
    ...
    @SpeakeasyMetadata({ elemType: XboxIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, XboxIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], XboxIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, XboxIdentityProviderData>;
