...
import { LinkedInApplicationConfiguration } from "./linkedinapplicationconfiguration";
import { ProviderLambdaConfiguration } from "./providerlambdaconfiguration";
import { Expose, Transform, Type } from "class-transformer";

export class LinkedInIdentityProviderData extends SpeakeasyBase {}

export class LinkedInIdentityProvider extends SpeakeasyBase {
    @SpeakeasyMetadata({ elemType: LinkedInApplicationConfiguration })
    @Expose({ name: "applicationConfiguration" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, LinkedInApplicationConfiguration> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], LinkedInApplicationConfiguration);
            }
            return obj;
        },
        { toClassOnly: true }
    ...
    @SpeakeasyMetadata({ elemType: LinkedInIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, LinkedInIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], LinkedInIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, LinkedInIdentityProviderData>;
