...
export class AppleIdentityProviderData extends SpeakeasyBase {}
...
export class AppleIdentityProvider extends SpeakeasyBase {
    ...
    @SpeakeasyMetadata({ elemType: AppleIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, AppleIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], AppleIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, AppleIdentityProviderData>;
