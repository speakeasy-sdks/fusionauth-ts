...
export class HYPRIdentityProviderData extends SpeakeasyBase {}
...
export class HYPRIdentityProvider extends SpeakeasyBase {
    ...
    @SpeakeasyMetadata({ elemType: HYPRIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, HYPRIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], HYPRIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, HYPRIdentityProviderData>;
