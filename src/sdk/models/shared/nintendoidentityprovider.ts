...
export class NintendoIdentityProviderData extends SpeakeasyBase {}
...
export class NintendoIdentityProvider extends SpeakeasyBase {
    ...
    @SpeakeasyMetadata({ elemType: NintendoIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, NintendoIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], NintendoIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, NintendoIdentityProviderData>;
