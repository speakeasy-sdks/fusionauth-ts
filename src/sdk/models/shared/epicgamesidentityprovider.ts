...
import { EpicGamesApplicationConfiguration } from "./epicgamesapplicationconfiguration";
...
export class EpicGamesIdentityProviderData extends SpeakeasyBase {}

/**
 * Epic gaming login provider.
 */
export class EpicGamesIdentityProvider extends SpeakeasyBase {
    @SpeakeasyMetadata({ elemType: EpicGamesApplicationConfiguration })
    @Expose({ name: "applicationConfiguration" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, EpicGamesApplicationConfiguration> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], EpicGamesApplicationConfiguration);
            }
            return obj;
        },
        { toClassOnly: true }
    ...
    @SpeakeasyMetadata({ elemType: EpicGamesIdentityProviderData })
    @Expose({ name: "data" })
    @Transform(
        ({ value }) => {
            const obj: Record<string, EpicGamesIdentityProviderData> = {};
            for (const key in value) {
                obj[key] = objectToClass(value[key], EpicGamesIdentityProviderData);
            }
            return obj;
        },
        { toClassOnly: true }
    )
    data?: Record<string, EpicGamesIdentityProviderData>;
